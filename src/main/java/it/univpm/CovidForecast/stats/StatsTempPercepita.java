...
/**
 * Classe che esegue le stats sulla temperatura percepita
 * 
 * @author 
 *
 */
	...
	public Vector<MeteoCitta> getStats(String tipoStat, Vector<MeteoCitta> vectPerStats) {
		...
		case "varianza":
			...
			for (int i = 0; i < vectPerStats.size(); i++) {
				vett.add(vectPerStats.elementAt(i).getTempPercepita());
			}
			n = vett.size();
			for (int i = 0; i < n; i++) {
			...
			mCVarianza.setTempPercepita(varianza);
		...
		case "media":
			...
			int num;
			for (int i = 0; i < vectPerStats.size(); i++) {
				vett1.add(vectPerStats.elementAt(i).getTempPercepita());
			}
			num = vett1.size();
			for (int i = 0; i < num; i++) {
			...
			mCMedia.setTempPercepita(media1);
